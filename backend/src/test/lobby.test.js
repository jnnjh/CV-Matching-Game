...
describe('GET /api/games/:gameId/players', () => {
  ...
  it('should return players for a valid game', async () => {
    const response = await request(app).get(`/api/games/${testGameId}/players`)
  ...
  it('should return 404 for invalid game', async () => {
    // Use a valid numeric ID that doesn't exist
    const fakeId = 999999
    const response = await request(app).get(`/api/games/${fakeId}/players`)
  ...
  it('should return empty array for empty lobby', async () => {
    ...
    const response = await request(app).get(`/api/games/${emptyGameId}/players`)
